...
export default function PreviewTable({
  ...
  return (
    <div className="w-full flex flex-col h-full max-h-[75vh] aura-card overflow-hidden">
      {/* Table Header Section */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 border-b border-[#27272A] bg-[#18181B] gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button
            onClick={onCancel}
            className="aura-button-secondary flex items-center justify-center gap-2 px-4 py-2 text-xs font-mono w-full sm:w-auto cursor-pointer"
          >
            <X className="w-4 h-4" />
            CANCEL_IMPORT
          </button>
          <button
            onClick={onConfirm}
            className="aura-button-primary flex items-center justify-center gap-2 px-6 py-2 text-xs font-mono w-full sm:w-auto cursor-pointer"
          >
            CONFIRM_AND_MAP
